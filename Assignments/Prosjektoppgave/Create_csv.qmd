...
count <- read.dta("G:/UiT_Fag/SOK-1005/Assignments/Prosjektoppgave/ccount.dta")

demo <- read.dta("G:/UiT_Fag/SOK-1005/Assignments/Prosjektoppgave/demo.dta")

UPC = read.csv('G:/UiT_Fag/SOK-1005/Assignments/Prosjektoppgave/upcsdr.csv')

mov = read.csv('G:/UiT_Fag/SOK-1005/Assignments/Prosjektoppgave/wsdr.csv')
```
...
mov_coca <- mov_coca %>% mutate(cola_sales = (PRICE*MOVE)/QTY)
mov_coca <- mov_coca %>% mutate(cola_profit = cola_sales*(PROFIT/100))


mov_coca <- mov_coca %>% group_by(STORE,WEEK) %>% 
  summarise(across(c(cola_sales,cola_profit),sum),
            .groups = 'drop') %>%
  as.data.frame()
...
mov_pepsi <- mov_pepsi %>% mutate(pepsi_sales = (PRICE*MOVE)/QTY)
mov_pepsi <- mov_pepsi %>% mutate(pepsi_profit = pepsi_sales*(PROFIT/100))

mov_pepsi <- mov_pepsi %>% group_by(STORE,WEEK) %>% 
  summarise(across(c(pepsi_sales,pepsi_profit),sum),
            .groups = 'drop') %>%
  as.data.frame()
...
mov_sprite <- mov_sprite %>% mutate(sprite_sales = (PRICE*MOVE)/QTY)
mov_sprite <- mov_sprite %>% mutate(sprite_profit = sprite_sales*(PROFIT/100))

mov_sprite <- mov_sprite %>% group_by(STORE,WEEK) %>% 
  summarise(across(c(sprite_sales,sprite_profit),sum),
            .groups = 'drop') %>%
  as.data.frame()
...
mov <- mov %>% mutate(soda_sales = cola_sales+pepsi_sales+sprite_sales)
mov <- mov %>% mutate(soda_profit = cola_profit+pepsi_profit+sprite_profit)
...
write.csv(mov_cust_demo, "G:\\UiT_Fag\\SOK-1005\\Assignments\\Prosjektoppgave\\mov_cust_demo.csv", row.names=FALSE)
```
